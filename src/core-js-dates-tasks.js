...
function getNextFriday(date) {
  ...
  const daysUntilFriday =
    (5 - nextDate.getUTCDay() + 7) % 7 === 0
      ? 7
      : (5 - nextDate.getUTCDay() + 7) % 7;
...
function isDateInPeriod(date, period) {
  ...
  return dateStart <= dateForCompare && dateForCompare <= dateEnd;
}
...
function getWeekNumberByDate(date) {
  const firstWeekYear = new Date(date.getFullYear(), 0, 1);

  return (
    1 +
    Math.round(
      ((date.getTime() - firstWeekYear.getTime()) / 86400000 -
        3 +
        ((firstWeekYear.getDay() + 6) % 7)) /
        7
    )
  );
}
...
function getNextFridayThe13th(date) {
  ...
  let year = newDate1.getUTCFullYear();
  let month = newDate1.getUTCMonth();
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let timeStart = new Date(reverser(period.start)).getTime();
  const timeEnd = new Date(reverser(period.end)).getTime() + 86400000;
  ...
  while (timeStart < timeEnd) {
    for (let i = 0; i < countWorkDays; i += 1) {
      const newDate = new Date(timeStart);
      const newString = formatter.format(newDate).split('.').join('-');
      res.push(newString);
      timeStart += 86400000;
    ...
    timeStart += countOffDays * 86400000;
  }
  return res;
}
